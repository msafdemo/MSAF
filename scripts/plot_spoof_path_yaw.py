...
def plot_parallel_dashed_lines(gt_x_positions, gt_y_positions, expected_deviation, line_color, subfolder):
    ...
    for i in range(1, len(gt_x_positions)):
        ...
        # add to list
        dashed_line_points.append((new_x1, new_y1))
        dashed_line_points.append((new_x2, new_y2))

        # draw dashed line and specify label
        line, = plt.plot([new_x1, new_x2], [new_y1, new_y2], linestyle='--', color='purple', label=label, linewidth=1.5)

        if label is not None:
            legend_handles_labels.append((line, label))

        # only want to show label once in the legend, so set label to None
        label = None
...
def add_spoofing_point_legend():
    ...
    red_dot, = plt.plot([], [], 'ro', markersize=5, label='Spoofing Point')
...
def save_and_show_plot(motion_state, last_y_values):
    ...
    plt.xlabel("Y Coordinate(m)", fontsize=36, labelpad=20)
    plt.ylabel("X Coordinate(m)", fontsize=36)
